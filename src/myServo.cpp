...
uint8_t MyServo::attach(int pin, uint16_t minUs, uint16_t maxUs)
{
  return attach(pin, minUs, maxUs, _valueUs);
}

uint8_t MyServo::attach(int pin, uint16_t minUs, uint16_t maxUs, int value)
{
  if (!_attached) {
    pinMode(pin, OUTPUT);
...
void MyServo::detach()
{
  if (_attached) {
    _servoMap &= ~(1 << _pin);
    // TODO
    startWaveform(_pin, REFRESH_INTERVAL, 0, 1);
...
void MyServo::writeMicroseconds(int value)
{
  ...
  if (_attached) {
    ...
    // TODO
    if (startWaveform(_pin, REFRESH_INTERVAL - _valueUs, _valueUs, 0, phaseReference))
    {
      _servoMap |= (1 << _pin);
    }
  }
